...
Persistence       SQLite (user profiles + routine shelf, via user_store.py)
...
│   │                         #   └─ RoutineProduct / CrossConflict / RoutineFit モデル
│   ├── config.py             # 閾値・モデルIDの集中管理
│   │                         #   └─ FUNCTION_GROUPS_PATH, GOAL_TO_FUNCTION
│   ├── conflicts.py          # 共有コンフリクトマトリクスヘルパー
│   │                         #   （auditor + routine_advisor が共有）
│   ├── user_store.py         # SQLiteユーザープロファイル + ルーティン棚
│   │                         #   └─ routine_productsテーブル / add・get・remove関数
│   └── nodes/
│       ├── scanner.py        # Flash & Pro VLMノード + 画像最適化
│       ├── registry.py       # ファジーレジストリマッチ（早期チェック + フルルックアップ）
│       ├── normalizer.py     # 原材料名 → 標準INCIキーへのマッピング
│       ├── auditor.py        # 安全性監査ノード（src.conflicts ローダーを使用）
│       ├── routine_advisor.py # クロス製品決定論的評価ノード（新規）
│       └── coach.py          # アドバイス生成（CoachResponse / ルーティンFit描画）
├── data/
...
│   ├── test_normalizer.py
│   ├── test_auditor.py
│   ├── test_routine_advisor.py  # クロス製品ロジック（新規）
│   ├── test_user_store.py       # ルーティン棚のCRUD（新規）
│   ├── test_coach.py
│   ├── test_websearch.py
│   └── test_routers.py
├── run_pipeline.py          # CLIエントリーポイント
│                            #   └─ --user-id / --add-to-routine フラグ・Fit出力
...
│       ├── auditor.py        # Safety audit node (refactored to use src.conflicts)
│       ├── routine_advisor.py # Deterministic cross-product evaluation node (new)
...
│   ├── function_groups.json     # Function/active category taxonomy (new)
...
│   │                        #   └─ seed also inserts routine products (incl. conflict pairs)
...
│   ├── test_normalizer.py
│   ├── test_auditor.py
│   ├── test_routine_advisor.py  # Cross-product logic (new)
│   ├── test_user_store.py       # Routine shelf CRUD (new)
│   ├── test_coach.py
│   ├── test_websearch.py
│   └── test_routers.py
├── run_pipeline.py          # CLI entry point
│                            #   └─ --user-id / --add-to-routine flag · Routine Fit output
