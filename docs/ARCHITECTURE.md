...
作者: **David Valls**（[GitHub](https://github.com/ShinBellator)）。SkinGraphは、「この製品は本当に自分の肌に合うのか、いま使っているものと両立するのか」という、繰り返し浮かぶシンプルな問いに、安全性データを任せられるだけの厳密さで答えるために作りました。フィードバック・Issue・コントリビューションを歓迎します。
...
| 🔭 **評価ハーネス** | `eval/evaluate.py` — 成分F1・バイリンガルブランド照合・NFKC正規化による精度計測 |
...
flowchart TD
    Start(["🚀 開始"])
    Quality["🚦 第1層 ピクセルゲート<br/>決定論的 · LLM不使用<br/>暗すぎ / 白飛び / 空白"]
    Classify["🔀 第2層 内容＋表裏判定<br/>Gemini Flash · 1コール<br/>製品か？ 何個か？"]
    Flash["⚡ Flashスキャナー<br/>Gemini Flash<br/>構造化出力"]
    Router{"🔀 推論ルーター<br/>信頼スコア閾値"}
    EarlyCheck["🔍 早期レジストリヒット<br/>99%マッチ閾値<br/>ループショートサーキット"]
    Correction["📝 修正フィードバックノード<br/>決定論的フィードバック生成<br/>次のプロンプトに注入"]
    Pro["🧠 Proスキャナー<br/>Gemini Pro<br/>空間推論フォーカス"]
    TagLang["🏷️ 言語タグ<br/>ラベル言語を記録<br/>（全言語受け付け）"]
    RegLookup["📚 レジストリマッチ<br/>90%マッチ閾値 · データヒーリング"]
    VerifyId["🔎 製品識別検証<br/>ブランド・製品名の読み取り<br/>信頼スコア判定"]
    WebSearch["🌐 Webサーチ<br/>成分検索<br/>（表面ラベル · レジストリ未登録品）"]
    ...
    Classify -->|"表面 · 単一製品"| VerifyId
    Classify -->|"製品なし / 複数 🚫"| Retake
    Flash --> Router

    Router -->|"conf ≥ 0.85 ✅"| TagLang
    Router -->|"0.50 ≤ conf < 0.85 ⚠️"| EarlyCheck
    Router -->|"conf < 0.50 🚨"| Pro

    EarlyCheck -->|"≥ 99% 一致 🎯<br/>LLMリトライをスキップ"| Normalizer
    EarlyCheck -->|"< 99% ミス"| Correction
    Correction -->|"批評注入<br/>反復回数 +1"| Flash

    Pro --> ProRouter
    ProRouter -->|"conf ≥ 0.85"| TagLang
    ProRouter -->|"失敗"| Retake
    Retake --> EndRetake

    TagLang --> RegLookup
    RegLookup -->|"マッチあり ✅"| Normalizer
    RegLookup -->|"マッチなし 🔍"| VerifyId

    VerifyId -->|"高信頼"| WebSearch
    VerifyId -->|"低信頼"| ConfirmId
    WebSearch -->|"成分取得 ✅"| Normalizer
    ...
    style TagLang fill:#e8d5f5,stroke:#6f42c1
    ...
    style VerifyId fill:#fff3cd,stroke:#ffc107
...
`GET /users/{id}/routine/dashboard`（ルーティンダッシュボード）、`GET /health`。
...
Built by **David Valls** ([GitHub](https://github.com/ShinBellator)). SkinGraph began as a way to answer a simple, recurring question — *"is this product actually right for my skin, and does it fit what I already use?"* — with a system rigorous enough to be trusted with safety-critical data. Feedback, issues, and contributions are welcome.
...
| 🔭 **Eval Harness** | `eval/evaluate.py` — field-level ingredient F1, bilingual brand/product match, NFKC normalization |
...
flowchart TD
    Start(["🚀 START"])
    Quality["🚦 Tier-1 Pixel Gate<br/>Deterministic · no LLM<br/>dark / blown-out / blank"]
    Classify["🔀 Tier-2 Content + Side<br/>Gemini Flash · one call<br/>product? how many?"]
    Flash["⚡ Flash Scanner<br/>Gemini Flash<br/>Structured Output"]
    Router{"🔀 Inference Router<br/>Confidence Thresholds"}
    EarlyCheck["🔍 Early Registry Hit<br/>99% Match Threshold<br/>Short-circuit loop"]
    ...
    TagLang["🏷️ Language Tag<br/>Record label language<br/>(all languages accepted)"]
    RegLookup["📚 Registry Match<br/>90% Match Threshold<br/>Data Healing"]
    VerifyId["🔎 Verify Identity<br/>Brand + product name read<br/>Confidence check"]
    WebSearch["🌐 Web Search<br/>Ingredient lookup<br/>(front labels · registry misses)"]
    ...
    Classify -->|"front · single product"| VerifyId
    ...
    EarlyCheck -->|"≥ 99% Match 🎯<br/>Skip LLM Retry"| Normalizer
    ...
    TagLang --> RegLookup
    RegLookup -->|"match found ✅"| Normalizer
    RegLookup -->|"no match 🔍"| VerifyId

    VerifyId -->|"confident"| WebSearch
    VerifyId -->|"uncertain"| ConfirmId
    WebSearch -->|"ingredients found ✅"| Normalizer
