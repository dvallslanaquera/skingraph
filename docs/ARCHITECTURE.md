...
| `poetry run python evaluate.py --model both` | 精度評価ハーネスを実行 |
...
`evaluate.py` を使用し、手動アノテーション済みグラウンドトゥルース（`data/ground_truth.json` — 日本語・韓国語・英語ラベル、表裏ショットを含む14枚）に対し評価。成分は**正規化後のINCI空間**でスコアリングする — 各抽出結果を翻訳・正規化レイヤー（VLMの`name_standardized` INCI ＋ 正規化レジャーのフォールバック）に通し、製品ごとに保存された`ingredient_inci`翻訳と照合する。これにより指標は**言語非依存**となり、韓国語ラベルも日本語ラベルと同じ基準でスコアリングされる。
...
| `poetry run python evaluate.py --model both` | Run the accuracy evaluation harness |
